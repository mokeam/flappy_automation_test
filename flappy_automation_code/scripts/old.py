...
def velCallback(msg):
    ...
    R_vx = Kd_x*np.exp(-0.02*np.abs(error_theta)*2) ###
    R_vy = Kd_y*error_theta

    #Error from the des10ired linear velocity
    E_vx = R_vx - msg.x
    E_vy = R_vy - msg.y

    # E_vx = free_space[0] - msg.x
    # E_vy = free_space[1] - msg.y

    # delta_error = [(E_vx - last_error[0])/dt,(E_vy - last_error[1])/dt]

    # acc  = np.array([K_acc_x*E_vx + Kd_x * delta_error[0], K_acc_y*E_vy + Kd_y * delta_error[1]])
    # acc  = np.array([K_acc_x*E_vx, K_acc_y*E_vy])
    acc  = np.array([p_gain*E_vx,p_gain*E_vy])
    acc = np.maximum(acc, - a_max)
    acc = np.minimum(acc, a_max)

    # last_error = [E_vx,E_vy]
   
    #Publish new acceleration command as Vector3
    pub_acc_cmd.publish(Vector3(acc[0], acc[1], 0.))
